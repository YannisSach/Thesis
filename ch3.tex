...
In the exploration phase, the exploration starts from $E.p$. The important part is calculation of the new sleep set at that step since some processes may have waken up.
...
As far as the algorithm is concerned it is clear that the most important part of the flow chart is the detection of dependencies. Any
modifications on the algorithm should mainly focus on the procedures taking place during the dependency detection.

\mediumGraph{flowchart.pdf}{Nidhugg's Flow Chart}
...
\begin{algorithm}
    ...
    \Fn{see\_events($seen\_access$)}{
        branches := seen\_access $ - \{ a \in seen\_access \mid a  \text{ happens before } last(E) \text{ or } \exists a' \in seen\_access \text{ which happens after } a \}$ \;
        update\_clocks() \;
...
\begin{algorithm}
    ...
    \Fn{add\_branch($b$)}{
        ...
        \ForEach{$e in dom(E')$}{
            \If{$b \text{ happens before } e \text{ or } \exists c \in candidates \mid c \text{ happens before } e $}{
                continue \;
            }
...
\begin{algorithm}
    ...
    \Fn{Explore($E$,$C$)}{
        \Let{s := last(S)}
        \For{all process $p$}{
            \If{$\exists i = max(\{ i \in dom(S) \mid S_i  \text{is dependent and may be co-enabled with } next(s,p) and i \not \leq  C(p)(proc(S_i)) \} $}{
...
\begin{algorithm}
    ...
    Explore($\langle \rangle$,$\emptyset$,$b$)\;
    \Fn{Explore($E$,$Sleep$,$b$)}{
        \If{$\exists p \in ((enabled(s_{[E]}) \backslash Sleep) \text{ and } B_v(E.p) <=b$}{
            backtrack(E) $:={p}$ \;
            \While{$\exists p \in (backtrack(E) \backslash Sleep \text { and } B_v(E.p) <=b$}{
                \ForEach{$e \in dom(E)$ such that $e \lesssim_{E.p} next_{[E]}(p)$}{
                    \Let{$E' = pre(E,e)$}
                    \Let{$u = notdep(e,E).p$}
                    \Let{$CI = \{ i \in I_{E'}(u) \mid i \rightarrow p \}$}
                    ...
                    \If{$CI \cap backtrack(E') = \emptyset$}{
                        ...
                        \Else{
                            add some $c(q') \in I_{[E'']}(u) \text{ to } backtrack(E'') $ \;
                        }
                    } 
                }
                \Let{ $Sleep' := {q \in Sleep \mid E \models p \diamondsuit q } $ } 
...
\begin{algorithm}[H]
    ...
    \Fn{see\_events($seen\_access$)}{
        ...
        branches := seen\_access $ - \{ a \in seen\_access \mid a  \text{happens before} last(E) or \exists a' \in seen\_access \text{ which happens after a } \}$ \;
        update\_clocks() \;
        \ForEach{$b \in branches$}{
            add\_branch($b$) \;
...
\begin{algorithm}[H]
    ...
    \Fn{add\_branch($b,is\_conservative$)}{
        ...
        $lc = null$ \;
        $lpc = null$ \;
        $E'$ := $E \text{ starting from } next_{E}(b)$ \; 
        \ForEach{$e \in dom(E')$}{
            \If{$b \rightarrow e \text{ or } \exists c \in candidates \mid c \rightarrow e $}{
                continue \;
            }
            ...
            \If{$e.pid \in backtrack(b) \text{ or } e.pid \in sleep\_set(b)$}{
                \If{$\text{not is\_conservative or} lc \rightarrow last(E)$}{
                    return \;
                }
            }
        }
        \If{$lpc \text{ and is\_conservative}$}{
            $backtrack(b) := backtrack(b) \cup lpc $
        }
        \Else{$backtrack(b) := backtrack(b) \cup lc $}
    }
