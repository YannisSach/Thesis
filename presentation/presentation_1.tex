...
\item In order to find an error of a concurrent program, one must examine every possible interleaving BUT leads to state
explosion.
\item Partial order reduction aims to reduce the number of interleavings explored by eliminating the exploration of
...
\begin{frame}
The implementation mainly is focused, as expected, % on  \verb|see_events()| and \verb|add_branches()| 
\end{frame}
