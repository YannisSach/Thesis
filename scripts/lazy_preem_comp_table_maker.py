...
file = open("LB_0.out", "r")
lb_text = file.read()
file.close()
lb_results = process_text(lb_text)

for i,_ in enumerate(lb_results):
	if lb_results[i][2] == "F" :
		lb_results[i] = (lb_results[i][0], lb_results[i][1], "0")

for b in range(1,5):
	file = open("LB_" + str(b) + ".out", "r")
	lb_text = file.read()
	file.close()
	temp_results = process_text(lb_text)
	for i,_ in enumerate(temp_results):
		if(lb_results[i][2] == "NF" and temp_results[i][2] == "F"):
			lb_results[i] = (temp_results[i][0], temp_results[i][1], str(b))

#print(pb_results)
#print(len(pb_results))
results = list(zip(pb_results,lb_results))
#print(len(results))
i = 0
for v in versions:
